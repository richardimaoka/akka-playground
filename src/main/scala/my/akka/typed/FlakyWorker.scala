      ...
      msg match {
        case Job(payload) =>
          if (ThreadLocalRandom.current().nextInt(50) == 0)
            throw new RuntimeException(s"Bad luck ${ctx.self}, ${count}")
...
object FlakyWorkerApp {
  ...
  def main(args: Array[String]): Unit = {
    val root = Actor.deferred[Nothing] { ctx =>
      ...
      /**
        * If used with ordinary ctx.spawn (i.e. withOUT Actor.supervise)
        * it will stop when the first exception is thrown
        * and will not be able to process any more messages after that.
        */
      ...
      val worker = ctx.spawn(
        Actor.supervise(workerBehavior).onFailure[RuntimeException](strategy), "worker")
